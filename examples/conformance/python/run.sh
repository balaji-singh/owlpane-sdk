...
node ../../_receiver/receiver.mjs --port-file "$PORTFILE" --service python-example --span hello-owlpane --timeout 45 & RPID=$!
...
# opentelemetry-instrument wires the SDK + exporter from the env vars; app.py has no Owlpane/OTel setup code.
OTEL_TRACES_EXPORTER=otlp OTEL_METRICS_EXPORTER=none OTEL_LOGS_EXPORTER=none .venv/bin/opentelemetry-instrument .venv/bin/python app.py
wait $RPID
